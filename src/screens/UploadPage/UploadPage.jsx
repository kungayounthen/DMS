...
function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    if (validExtensions.includes(file.type)) {
      ...
      ).then(res=>{console.log('successful',res); res.json()})
  ...
  return (
    <Box component='main' className="flex flex-1">
      ...
      <Box className='self-center w-3/5 transparent mx-auto container py-4 px-8 rounded-md shadow-lg shadow-slate-500'>
        ...
        <form ref={formRef}>
          <Box className='flex flex-1 justify-around gap-3 mt-5'>
            ...
            <Box className='flex flex-col flex-1'>
              ...
              <input type="text" id="name" name='name' value={name} className="h-12 rounded-md pl-4" onChange={event => setName(event.target.value)} />
            </Box>
          </Box>
        ...
        <button type="submit" className="bg-black" onClick={fileCheck}>check file data</button>
      </Box>
    </Box>
  )
}
